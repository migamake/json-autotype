...
module Data.Aeson.AutoType.CodeGen.ElixirFormat(
...
-- second element of the triple is the mapped identifier name in Elixir.
-- third element of the triple shows the type in a formatted way
type MappedKey = (Text, Text, Text, Bool)

-- | Make ToJSON declaration, given identifier (object name in Elixir) and mapping of its keys
-- from JSON to Elixir identifiers *in the same order* as in *data type declaration*.
...
-- | Make ToJSON declaration, given identifier (object name in Elixir) and mapping of its keys
-- from JSON to Elixir identifiers in the same order as in declaration
...
-- 1. Treating all characters that are not acceptable in Elixir variable name as end of word.
-- 2. Capitalizing each word, but a first (camelCase).
-- 3. Adding underscore if first character is non-alphabetic.
-- 4. Escaping Elixir keywords if the whole identifier is such keyword.
